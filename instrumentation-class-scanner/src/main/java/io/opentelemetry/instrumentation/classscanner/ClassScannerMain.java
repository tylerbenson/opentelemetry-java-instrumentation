...
public class ClassScannerMain {
  ...
  public static void main(String[] args) throws IOException {
    if(args.length == 0) {
      ...
      System.out.println("java -jar opentelemetry-instrumentation-class-scanner.jar [list of jars or class roots]");
    ...
    List<String> paths = Arrays.stream(args).map(arg -> arg.replace("production", "test"))
    ...
    analyzedClasses.stream()
        .filter(analyzedClass -> !analyzedClass.methodsWithSynchronize.isEmpty()).forEach(
            analyzedClass -> System.out.println("\t" +
                analyzedClass.className + analyzedClass.methodsWithSynchronize));

    System.out.println("Client invocation methods:");
    analyzedClasses.stream()
        .filter(analyzedClass -> !analyzedClass.methodsWithClientCalls.isEmpty()).forEach(
            analyzedClass -> System.out.println("\t" +
                analyzedClass.className + analyzedClass.methodsWithClientCalls));

    System.out.println("Repository invocation methods:");
    analyzedClasses.stream()
        .filter(analyzedClass -> !analyzedClass.methodsWithRepositoryCalls.isEmpty()).forEach(
            analyzedClass -> System.out.println("\t" +
                analyzedClass.className + analyzedClass.methodsWithRepositoryCalls));

    System.out.println("High call count methods:");
    analyzedClasses.stream()
        .filter(analyzedClass -> !analyzedClass.methodsWithHighMethodCount.isEmpty()).forEach(
            analyzedClass -> System.out.println("\t" +
                analyzedClass.className + analyzedClass.methodsWithHighMethodCount));

    System.out.println("High branch count methods:");
    analyzedClasses.stream()
        .filter(analyzedClass -> !analyzedClass.methodsWithHighBranchCount.isEmpty()).forEach(
            analyzedClass -> System.out.println("\t" +
                analyzedClass.className + analyzedClass.methodsWithHighBranchCount));
