package io.opentelemetry.instrumentation.classscanner;
...
public class AnalyzedClass {
  private static final int HIGH_METHOD_INSTRUCTION_COUNT = 100;
  private static final int HIGH_BRANCH_INSTRUCTION_COUNT = 15;

  private static final Set<String> BASIC_METHOD = new HashSet<>(
      Arrays.asList("<init>", "<clinit>", "toString", "hashCode", "equals"));
  ...
  public AnalyzedClass(String className, InputStream resourceAsStream) throws IOException {
    ...
    methodsWithSynchronize = analyzedMethods.stream().filter(AnalyzingMethodVisitor::notBasicMethod)
        .filter(AnalyzingMethodVisitor::hasSynchronized).map(AnalyzingMethodVisitor::name)
        .collect(Collectors.toSet());
    methodsWithClientCalls = className.endsWith("Client") ? Collections.emptySet()
        : analyzedMethods.stream().filter(AnalyzingMethodVisitor::notBasicMethod)
            .filter(AnalyzingMethodVisitor::hasClientCall).map(AnalyzingMethodVisitor::name)
            .collect(Collectors.toSet());
    methodsWithRepositoryCalls = className.endsWith("Repository") ? Collections.emptySet()
        : analyzedMethods.stream()
            .filter(AnalyzingMethodVisitor::notBasicMethod)
            .filter(AnalyzingMethodVisitor::hasRepositoryCall).map(AnalyzingMethodVisitor::name)
            .collect(Collectors.toSet());
    methodsWithHighMethodCount = analyzedMethods.stream()
        .filter(AnalyzingMethodVisitor::notBasicMethod)
        .filter(visitor -> visitor.methodCallCount >= HIGH_METHOD_INSTRUCTION_COUNT)
        .map(AnalyzingMethodVisitor::name).collect(Collectors.toSet());
  ...
  public boolean isInteresting() {
    return !(methodsWithSynchronize.isEmpty() && methodsWithClientCalls.isEmpty()
        && methodsWithRepositoryCalls.isEmpty() && methodsWithHighMethodCount.isEmpty()
  ...
  private static class AnalyzingMethodVisitor extends MethodVisitor {
    ...
    protected AnalyzingMethodVisitor(String name, int access) {
      ...
      this.name = name;
      isBasicMethod =
          BASIC_METHOD.contains(name) || name.startsWith("get") || name.startsWith("set")
              || name.startsWith("is") || name.startsWith("lambda$");
      hasSynchronized = (access & ACC_SYNCHRONIZED) == ACC_SYNCHRONIZED;
    }

    @Override
    public String toString() {
      return "AnalyzingMethodVisitor{" + "name='" + name + '\'' + ", hasSynchronized="
          + hasSynchronized + ", methodCallCount=" + methodCallCount + ", jumpCount=" + jumpCount
          + '}';
    }
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String descriptor,
      ...
      if (!owner.equals(returnType)) {
        hasClientCall |=
            owner.endsWith("Client") && !returnType.endsWith("Client") && !returnType.endsWith(
                "Builder");
        hasRepositoryCall |= owner.endsWith("Repository") && !returnType.endsWith("Repository")
